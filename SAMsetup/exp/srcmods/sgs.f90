...
subroutine sgs_scalars()
  ...
  implicit none
    ...
    real flx_tmp(0:nx, ny, 0:nzm)
    integer k


      call diffuse_scalar(t,fluxbt,fluxtt,tdiff,tdiff_xy,twsb,flx_t, &
      ...
      if(advect_sgs) then
         call diffuse_scalar(tke,fzero,fzero, dummy, dummy,sgswsb,flx_tmp, &
        ...
        if(   k.eq.index_water_vapor             &! transport water-vapor variable no metter what
         ...
         .or. doprecip.and.flag_precip(k).eq.1 ) then
           ...
           call diffuse_scalar(micro_field(:,:,:,k),fluxbtmp,fluxttmp, &
                mkdiff(:,k),mkdiff_xy(:,:,:,k), mkwsb(:,k), flx_tmp, dummy,dummy,dummy,.false.)
           !!!call diffuse_scalar(micro_field(:,:,:,k),fluxbtmp,fluxttmp, &
           !!!     mkdiff(:,k),dummy, mkwsb(:,k), flx_tmp, dummy,dummy,dummy,.false.)
           !!! Da modified
           !!!call diffuse_scalar(micro_field(:,:,:,k),fluxbtmp,fluxttmp, &
      ...
      if(dotracers) then
        ...
        do k = 1,ntracers
          ...
          call diffuse_scalar(tracer(:,:,:,k),fluxbtmp,fluxttmp, &
               trdiff(:,k),dummy, trwsb(:,k),flx_tmp, &
               dummy,dummy,dummy,.false.)
          !!!call diffuse_scalar(tracer(:,:,:,k),fluxbtmp,fluxttmp, &
          !!!     trdiff(:,k),trwsb(:,k), &
          !!!     dummy,dummy,dummy,.false.)
