subroutine diffuse_scalar (f,fluxb,fluxt, &
                          fdiff, fdiff_xy, flux,flx, f2lediff,f2lediss,fwlediff,doit)
...
real flux(nz)
real flx(0:nx,1,0:nzm)      ! added by Da
real fdiff_xy(nx,ny,nzm)      ! added by Da
real fdiff(nz)
real f2lediff(nzm)
real f2lediss(nzm)
real fwlediff(nzm)
...
if(RUN3D) then
  ...
  call diffuse_scalar2D (f,fluxb,fluxt,tkh,rho,rhow,flux, flx)
endif
...
if(dostatis) then
  ...
  do k=1,nzm
    ...
    do j=1,ny
     do i=1,nx
      fdiff_xy(i,j,k) = f(i,j,k)-df(i,j,k)
