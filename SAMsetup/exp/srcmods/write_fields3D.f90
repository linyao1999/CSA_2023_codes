...
use microphysics, only: nmicro_fields, micro_field, flag_number, &
     ...
     index_water_vapor, GET_reffc, Get_reffi, mkfname, mkflongname, &
     mkfunits
implicit none
...
if(compute_reffi.and.(dolongwave.or.doshortwave).and.rad3Dout) then
  ...
  do k=1,nzm
   do j=1,ny
    do i=1,nx
      tmp(i,j,k)=tdiff_xy(i,j,k)*86400./dtn
    end do
   end do
  end do
  name='Ft'
  long_name='Sub-grid tendency of MSE'
  units='K/day'
  call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
                                 save3Dbin,dompi,rank,nsubdomains)
...
do n = 1,nmicro_fields
   !!!if(docloud.AND.flag_micro3Dout(n).gt.0.AND.n.ne.index_water_vapor) then
      ...
      do k=1,nzm
         do j=1,ny
            do i=1,nx
               tmp(i,j,k)=mkdiff_xy(i,j,k,n)*rhow(k)*lcond/dtn
      ...
      name=TRIM(mkfname(n))
      long_name=TRIM(mkflongname(n))
      units=TRIM(mkfunits(n))
      call compress3D(tmp,nx,ny,nzm,name,long_name,units, &
           save3Dbin,dompi,rank,nsubdomains)
   !!!end if
end do
